...
namespace CTSTools.BLL.Common.Files
{
    public class File_Service
    {
        ...
        public static ValidationResultDTO DeleteFolder(FileDTO FileDTO)
        {
            var _ValidationResultDTO = new ValidationResultDTO
            {
                Description = "The record has been  successfully deleted"
            ...
            try
            {
                ...
                if (Directory.Exists(FileDTO.URL))
                {
                    Directory.Delete(FileDTO.URL, true);
                }
            }
            catch (Exception ex)
            {
        ...
        public static ValidationResultDTO FileSize_Validation(FileDTO FileDTO)
        {
            ...
            try
            {
                ...
                if (Convert.ToInt32(FileDTO.Size) > (int)FileLimit_Enum.MB_5)
                {
        ...
        public static FileDTO GetFile(FileDTO FileDTO)
        {
            try
            {
                ...
                if (Directory.Exists(FileDTO.URL))
                {
                    ...
                    FileDTO = GetVirtualDirectory(FileDTO).Data;
                    FileDTO.URL = String.Format("{0}/{1}", FileDTO.URL, FileDTO.ID);
                    if (FileDTO.DirectoryArray.Count() > 0)
                    {
                        foreach (var _directory in FileDTO.DirectoryArray)
                        {
                            FileDTO.Name = Path.GetFileNameWithoutExtension(_directory);
                            FileDTO.Extension = Path.GetExtension(_directory);
                            //FileDTO.Size = new FileInfo(_directory).Length;
                            //FileDTO.Icon = GetIconURLByFileExtension(FileDTO);
                            FileDTO.URL = string.Format("{0}\\{1}", FileDTO.URL, Path.GetFileName(_directory)).Replace("\\", "/");
                        }
                    }
                    else
                    {
                        FileDTO.URL = ConfigurationManager.AppSettings["NoImageURL"];
                    }
                }
                else
                {
            ...
            try
            {
                ...
                if (Directory.Exists(FileDTO.URL))
                {
                    ...
                    foreach (var _directory in FileDTO.DirectoryArray)
                    {
                        ...
                        //_fileDTO.Name = (_fileDTO.Name.Length >= 15) ?_fileDTO.Name.Substring(0,15)+"...": _fileDTO.Name;
        ...
        public static ValidationResultDTO GetVirtualDirectory(FileDTO FileDTO)
        {
            var _ValidationResultDTO = new ValidationResultDTO();
            try
            {
                ...
                {
                    case (int)FileDirectory_Enum.ItemHeaderPictureDirectory:
                        FileDTO.URL = ConfigurationManager.AppSettings["VirtualItemHeaderPictureDirectory"].ToString();
                        break;
                    case (int)FileDirectory_Enum.ItemHeaderAttachmentsDirectory:
                        FileDTO.URL = ConfigurationManager.AppSettings["VirtualItemHeaderAttachmentDirectory"].ToString();
                        break; ;
                    case (int)FileDirectory_Enum.ItemLineAttachmentsDirectory:
                        FileDTO.URL = ConfigurationManager.AppSettings["VirtualItemLineAttachmentDirectory"].ToString();
                        break;
                }
                _ValidationResultDTO.Data = FileDTO;
            }
        ...
        public static DirectoryDTO GetDirectoryInformation(DirectoryDTO DirectoryDTO)
        {
            var _directoryDTO = new DirectoryDTO();
            if (DirectoryDTO.OriginURL != null)
            {
                _directoryDTO.OriginURL = DirectoryDTO.OriginURL;
                _directoryDTO.OriginURL = _directoryDTO.OriginURL.Replace("/", "\\");
                _directoryDTO.OriginURL = _directoryDTO.OriginURL.Replace("Documents", "WebDocuments\\ImportLogDocuments").Replace("avmx-s05:8068", "avmx-s05");
                _directoryDTO.OriginURL = (_directoryDTO.OriginURL.Contains("http:")) ? _directoryDTO.OriginURL.Remove(0, 5) : _directoryDTO.OriginURL;
            }
            else
            {
                _directoryDTO.OriginURL = ConfigurationManager.AppSettings["LetterAttachmentDirectory"].ToString() + DirectoryDTO.Name;
                DirectoryDTO.OriginURL = ConfigurationManager.AppSettings["VirtualLetterAttachmentDirectory"].ToString() + DirectoryDTO.Name;
            }
            if (Directory.Exists(_directoryDTO.OriginURL))
            {
                DirectoryDTO.DocumentList = Directory.GetFileSystemEntries(_directoryDTO.OriginURL);

                foreach (var _document in DirectoryDTO.DocumentList)
                {
                    var _fileDTO = new FileDTO();
                    _fileDTO.Name = Path.GetFileNameWithoutExtension(_document);
                    _fileDTO.Extension = Path.GetExtension(_document);
                    _fileDTO.Type = (_fileDTO.Extension != string.Empty) ? "File" : "Folder";
                    _fileDTO.FileName = _fileDTO.Name + _fileDTO.Extension;
                    _fileDTO.Icon = GetIconURLByFileExtension(_fileDTO);
                    _fileDTO.URL = string.Format("{0}/{1}", DirectoryDTO.OriginURL, Path.GetFileName(_document)).Replace("\\", "/");
                    _directoryDTO.FileList.Add(_fileDTO);
                }
                _directoryDTO.OriginURL = DirectoryDTO.OriginURL;
                DirectoryDTO.OriginURL = DirectoryDTO.OriginURL.Replace("//", "/").Replace("http:/", "http://");
                _directoryDTO.PreviousURL = (DirectoryDTO.OriginURL == ConfigurationManager.AppSettings["VirtualLetterAttachmentDirectory"].ToString() + DirectoryDTO.Name) ? "" : Path.GetDirectoryName(_directoryDTO.OriginURL.Replace("/", "\\").TrimEnd(Path.DirectorySeparatorChar)) + Path.DirectorySeparatorChar;
                _directoryDTO.PreviousURL = _directoryDTO.PreviousURL.Replace("\\", "//");

                Uri _originURL = new Uri(DirectoryDTO.OriginURL);
                _directoryDTO.Name = _originURL.Segments.Last().Replace("/", "").Replace("%20", " ");
            }
            return _directoryDTO;
        }
        #endregion
    }
}
