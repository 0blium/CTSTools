...
public class User_PermissionMap
{
    ...
    public static List<User_PermissionXPO> DTOListToXPOList(List<User_PermissionDTO> User_PermissionDTOList, UnitOfWork UnitOfWork)
    {
        var _xPOList = new List<User_PermissionXPO>();
        try
        {
            foreach (var _user_permissionDTO in User_PermissionDTOList)
            {
                _xPOList.Add(DTOtoXPO(_user_permissionDTO, UnitOfWork));
            }
        }
        catch (Exception ex)
        {
