...
public class Item_SupportGroup_Repository
{
    ...
    public static ValidationResultDTO CreateMultiple(List<Item_SupportGroupDTO> Item_SupportGroupDTOList)
    {
        var _validationResultDTO = new ValidationResultDTO
        {
            Description = "The record has been deleted successfully."
        ...
        {
            using var _unit = XPO_Helper.GetNewUnitOfWork();
            var _xPOList = Item_SupportGroupMap.DTOListToXPOList(Item_SupportGroupDTOList, _unit);
            _unit.Save(_xPOList);
            _unit.CommitChanges();
