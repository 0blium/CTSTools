...
public class DashboardLine_Repository
{
    ...
    public static ValidationResultDTO CreateMultiple(List<DashboardLineDTO> DashboardLineDTOList)
    {
        var _validationResultDTO = new ValidationResultDTO
        {
            Description = "The record has been deleted successfully."
        ...
        {
            using var _unit = XPO_Helper.GetNewUnitOfWork();
            var _xPOList = DashboardLineMap.DTOListToXPOList(DashboardLineDTOList, _unit);
            _unit.Save(_xPOList);
            _unit.CommitChanges();
        }
        catch (Exception ex)
