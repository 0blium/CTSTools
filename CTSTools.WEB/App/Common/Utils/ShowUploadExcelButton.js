...
export async function ShowUploadMassiveButton(UserID) {
    ...
    if (_user_PermissionList == null || _user_PermissionList.length === 0)
        return;
    // If you have permissions, we will filter the list to get elements that have Massive in their ActioneName
    _user_PermissionList = _user_PermissionList.filter(User_PermissionDTO => User_PermissionDTO.PermissionDTO.ActionName === "Massive");
    if (_user_PermissionList == null || _user_PermissionList.length === 0)
        return;
    ...
    // We perform a forEach to compare the id names of our _buttonList list with our _moduleNameList list and show the button if it has Massive permission
